...
describe('general api tests', ()=>{
  ...
  describe('POST request tests', () => {
    ...
    test('POST creates a new blog post using provided data', async () => {
      ...
      expect(response.body[response.body.length-1].title).toEqual('POST tests')
    })

    test('fail when \'likes\' property is missing', async () => {
      await api.post('/api/blogs').send(newObjectWithoutLikes).set('Authorization', `Bearer ${token}`).expect(201)
      const response = await api.get('/api/blogs')
      expect(response.body[response.body.length-1].likes).toEqual(0)
    })

    test('fail when \'title\' and \'url\' properties are missing', async () => {
      await api.post('/api/blogs').send(newObjectWithoutUrlTitle).set('Authorization', `Bearer ${token}`).expect(400)
    })
  ...
  describe('DELETE request tests', () => {
    test('DELETE removes specified blog post provided authorization', async () => {
      ...
      await api.del('/api/blogs/'+blogToDelete.id).set('Authorization', `Bearer ${token}`).expect(204)
      const responseAfterDeletion = await helper.blogsInDB()

      expect(responseAfterDeletion.length).toEqual(helper.initialBlogs.length-1)
  ...
  describe('PUT request tests', () => {
    ...
    const putTest = async (data) => {
      ...
      await api.put('/api/blogs/'+blogsBeforeUpdate[0].id).send(data).expect(200)
